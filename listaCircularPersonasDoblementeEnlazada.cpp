...
public:
    ...
    if (head == nullptr) {  // la lista vacía
        head = tail = newNode;
        head->next = head->prev = head;
    } else {  // si la lista no está vacía
        Nodo<T>* actual = head;

        // iIncrementar el ID de todos los nodos existentes
        do {
            actual->data.ID++;
            actual = actual->next;
        } while (actual != head);

        // Actualización de punteros para una lista doblemente enlazada
        newNode->next = head;       
        newNode->prev = tail;       
        head->prev = newNode;       
        tail->next = newNode;      
    ...
    contadorID++;  // incrementar el contador de IDs global
    }
